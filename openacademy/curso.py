...
class openacademy_session(osv.Model):
    ...
    def onchange_taken_seats(self, cr, uid, ids, seats, attendee_ids):
        ...
        resultado={'value':{'taken_seats_percent':self._get_percent(seats, num_attendees)}}
    ...
    def _set_hours(self, cr, uid, id, field, value, arg, context=None):
        if value:
            print float(value /8.0),"antess"
            self.write(cr, uid, id,{'duration' : float(value / 8.0)},context=context)
    ...
    _columns={
        ...
        'seats': fields.integer(string='Numero de Ascientos'),
        ...
        'attendee_ids': fields.one2many('openacademy.attendee', 'session_id', string='Assistentes'),
        ...
        'state': fields.selection([('draft','Draft'),('confirm','Confimred'),('done','Done')], string='State'),
    }
